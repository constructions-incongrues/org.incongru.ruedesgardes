...
function variable_product_type_options() {
	...
	jQuery(function(){
		...
		jQuery('#variable_product_options').on('click', 'button.link_all_variations', function(){
			...
			var answer = confirm('<?php esc_js(translate( 'Are you sure you want to link all variations? This will create a new variation for each and every possible combination of variation attributes (max 50 per run).', 'woocommerce' )); ?>');
			...
			if (answer) {
				...
				jQuery.post('<?php echo admin_url('admin-ajax.php'); ?>', data, function(response) {
					...
					if (count==1) {
						alert( count + ' <?php esc_js(translate( "variation added", 'woocommerce' )); ?>');
					} else if (count==0 || count>1) {
						alert( count + ' <?php esc_js(translate( "variations added", 'woocommerce' )); ?>');
					} else {
						alert('<?php esc_js(translate( "No variations added", 'woocommerce' )); ?>');
					}
		...
		jQuery('#variable_product_options').on('click', 'button.remove_variation', function(e){
			e.preventDefault();
			var answer = confirm('<?php esc_js(translate( 'Are you sure you want to remove this variation?', 'woocommerce' )); ?>');
			if (answer){
		...
		jQuery('.wc-metaboxes-wrapper').on('click', 'a.bulk_edit', function(event){
			...
			if ( field_to_edit == 'toggle_enabled' ) {
				...
				var answer = confirm('<?php esc_js(translate( 'Are you sure you want to delete all variations? This cannot be undone.', 'woocommerce' )); ?>');
				if (answer){

					var answer = confirm('<?php esc_js(translate( 'Last warning, are you sure?', 'woocommerce' )); ?>');
				...
				var value = prompt("<?php esc_js(translate( 'Enter a value', 'woocommerce' )); ?>");
		...
		jQuery('#variable_product_options').on('click', '.upload_image_button', function( event ) {
			...
			if ( $button.is('.remove') ) {
				...
				variable_image_frame = wp.media.frames.variable_image = wp.media({
					...
					title: '<?php esc_js(translate( 'Choose an image', 'woocommerce' )); ?>',
					button: {
						text: '<?php esc_js(translate( 'Set variation image', 'woocommerce' )); ?>'
					}
				});
